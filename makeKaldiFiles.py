...
#split_method = "random_percentage" #random_percentage, random_number, corpusfile, speaker, list, None ..
test_percentage = 2
...
def validate_corpusfile(corpusfile):
    ...
    lines = fh.readlines()
    print("\nFile: %s, lines: %d" % (corpusfile,len(lines)))
    linenr = 0
    fileok = True
    okcounter = 0
    for line in lines:
        ...
        text_re = u"[A-ZÁÉÍÓÚa-záéíóú0-9 '|.,?!-]+"
        trans_re = u"[a-z@# _.012-]+"
        ...
        try:    
            fields = line.split("\t")
            #print("len(fields): %d" % len(fields))
            if len(fields) == 5:
                (fileid,speaker,wavfile,text,trans) = fields
            elif len(fields) == 6:
                (fileid,speaker,gender,wavfile,text,trans) = fields
            elif len(fields) == 7:
                (fileid,speaker,gender,wavfile,text,trans,comment) = fields
            else:
                ok = False
        ...
        #Fix for cillairne speaker id's that kaldi doesn't like
        m = re.match("^([ab])([0-9]+)$", speaker)
        if m:
            newspeaker = "%s%03d" % (m.group(1), int(m.group(2)))
            newfileid = re.sub("^%s" % speaker, newspeaker, fileid)
            speaker = newspeaker
            fileid = newfileid

        if trans == "":
            ok = False
            print("Error: empty transcription in line %s" % line)
        ...
        if not ok and exit_on_first_error:
            ...
            if speaker not in spk2gender_dict:
                if gender in ["f", "m"]:
                    spk2gender_dict[speaker] = gender
                else:
                    print("ERROR: speaker %s is not in spk2gender file %s, and gender is %s" % (speaker, spk2gender_file, gender))
                    ok = False
                    if exit_on_first_error:
                        sys.exit(1)
            ...
            text = text.split(" ")
            trans = trans.split(" # ")
            trans = [value for value in trans if value != "sil"]

            if not len(text) == len(trans):
                print("ERROR in %s line %s: text and transcription are not equal length!\nText  (%d): %s\nTrans (%d): %s" % (corpusfile, linenr, len(text), "\t|\t".join(text), len(trans), "\t|\t".join(trans)))
                print("Skipping line %d (text and transcription don't match)" % (linenr))
                ok = False
                if exit_on_first_error:
                    sys.exit(1)
        ...
        if comment and "delete" in comment:
            print("Skipping line %d (delete comment)" % (linenr))
            ok = False

        (trans, msg) = validateTrans(trans)
        if not trans:
            print("ERROR in %s line %s: INVALID TRANSCRIPTION %s: %s" % (corpusfile, linenr, trans, msg))
            ok = False
            if exit_on_first_error:
                sys.exit(1)
        ...
        if ok == False:
            ...
            path_to_wavfile =  wavfile
            corpusdict[fileid] = (speaker,path_to_wavfile,text,trans)
            okcounter += 1
    print("OK lines in %s: %d" % (corpusfile, okcounter))
    return fileok
    
def validateTrans(trans):
    newtrans = [] 
    for t in trans:
        #remove stress levels and syllable boundaries
        t = re.sub("(^| )([0-9.] )+"," ", t).strip()
        newtrans.append(t)
    return (newtrans, True)
...
def splitTrainTest():
    ...
    if split_method == None:
        traindict["dict"] = corpusdict
    elif split_method == "random_percentage":
...
def writeKaldiLanguageFiles(expdir):
    ...
    for text in kaldi_corpus:
        fh.write(u"%s\n" % " ".join(text))
    fh.close()
    print("\nWritten %d lines to %s" % (len(kaldi_corpus),path_to_kaldi_corpus_file))
